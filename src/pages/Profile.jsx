...
export default function Profile() {
  ...
  return (
    <div className="bg-surface-container-lowest text-on-surface min-h-[100dvh] pb-28 relative overflow-x-hidden">
      ...
      {/* Header */}
      <header className="relative pt-10 pb-6 px-6 max-w-2xl mx-auto flex items-center justify-between z-10">
        <div className="flex items-center gap-4">
          {session.user.user_metadata?.avatar_url ? (
            ...
            <div className="w-14 h-14 rounded-full bg-surface-container-highest flex items-center justify-center ghost-border">
          ...
          <div>
            ...
            <p className="text-on-surface-variant text-sm font-body">Library Collection</p>
          </div>
        </div>

        <button
          onClick={signOut}
          className="flex items-center justify-center w-10 h-10 rounded-full glass-panel ghost-border hover:text-error transition-all"
          title="Sign Out"
        >
        ...
        <div className="flex p-1 glass-panel rounded-2xl">
          {['movies', 'actors'].map((tab) => (
            ...
            >
              ...
              <span className="relative z-10 capitalize font-label">{tab}</span>
            </button>
          ))}
        </div>
      </div>
                      ...
                      {movies.map((movie) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="absolute top-2 right-2 w-8 h-8 rounded-full glass-panel flex items-center justify-center hover:bg-error/80 transition-colors"
                          >
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-4 gap-6">
                    <AnimatePresence>
                      {actors.map((actor) => (
                        ...
                        >
                          <div className="w-24 h-24 rounded-full overflow-hidden glass-panel ghost-border mb-3 relative">
                            {actor.profile_path ? (
                              <img
...
function EmptyState({ type, onExplore }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 px-6 text-center glass-panel rounded-2xl ghost-border">
      ...
      <p className="text-on-surface-variant mb-8 max-w-[260px] leading-relaxed font-body">
        Your library is empty. Start exploring to find {type === 'movies' ? 'films' : 'artists'} you love.
      </p>
      <button onClick={onExplore} className="relative overflow-hidden neon-btn px-8 py-4 text-sm flex items-center gap-2">
        <motion.div
          animate={{ x: ['-200%', '200%'] }}
